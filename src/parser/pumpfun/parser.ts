...
export class PumpFunParser implements BaseParser<PumpFunTransaction> {
    ...
    parse(transaction: ParsedTransactionWithMeta): PumpFunTransaction {
        ...
        for (const ix of instructions) {
            ...
            if (discriminator.equals(this.buyDiscriminator)) {
                transactionResult.actions.push(this.getTradeAction(transaction, ix as PartiallyDecodedInstruction, "buy"))
            } else if (discriminator.equals(this.sellDiscriminator)) {
                transactionResult.actions.push(this.getTradeAction(transaction, ix as PartiallyDecodedInstruction, "sell"))
            }
        }
    ...
    getTradeAction(transaction: ParsedTransactionWithMeta, instruction: PartiallyDecodedInstruction, type: 'buy' | 'sell'): PumpFunAction {
        ...
        switch (type) {
            case 'buy':
                tradeData.amountIn = BigInt(solAmount)
                tradeData.amountOut = amount
            case 'sell':
                tradeData.amountIn = amount
                tradeData.amountOut = BigInt(solAmount)
        }
        return {
            type,
            info: tradeData
    ...
    getInstructions(transaction: ParsedTransactionWithMeta) {
        return transaction.transaction.message.instructions.filter((ix)=>ix.programId.toString() == PumpIDL.metadata.address)
    }
    ...
    getAccounts(instruction: PartiallyDecodedInstruction) {
        return {
            ...
            traderAta: instruction.accounts[5],
